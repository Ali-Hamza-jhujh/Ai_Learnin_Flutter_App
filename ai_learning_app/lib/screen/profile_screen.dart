...
// 2. XP + streak stats
// 3. Study stats (notes, tests, avg score)
...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  double get _avgScore =>
      (_stats?['averageTestScore'] as num?)?.toDouble() ?? 0.0;
  ...
  List<Map<String, dynamic>> _getAchievements() => [
        {
          'emoji': '📝',
          'title': 'Note Taker',
          'unlocked': _notesCount >= 1,
          'color': AppColors.violet
        },
        {
          'emoji': '🎯',
          'title': 'Quiz Master',
          'unlocked': _testsCount >= 5,
        ...
        },
        {
          'emoji': '🏆',
          'title': 'Level 5',
          'unlocked': _levelNum >= 5,
          'color': const Color(0xFFFF6B6B)
        },
        {
          'emoji': '👑',
          'title': 'Level 10',
          'unlocked': _levelNum >= 10,
          'color': AppColors.gold
        },
      ];

  Widget _achievementBadge(
      String emoji, String title, bool unlocked, Color color) {
    return Container(
        width: 80,
        padding: const EdgeInsets.symmetric(vertical: 16, horizontal: 8),
        decoration: BoxDecoration(
            color: unlocked ? color.withValues(alpha: 0.1) : AppColors.bgCard,
            borderRadius: BorderRadius.circular(18),
            border: Border.all(
                color: unlocked
          ...
          Stack(alignment: Alignment.center, children: [
            Text(emoji,
                style: TextStyle(
                    fontSize: 28,
                    color:
                        unlocked ? null : Colors.white.withValues(alpha: 0.3))),
            if (!unlocked)
              const Positioned(
                  bottom: 0,
                  right: 0,
    ...
    return Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          GlassCard(
            ...
            _divider(),
            _settingRow(Icons.key_rounded, 'API Keys',
                'Gemini, Groq, Cerebras', AppColors.gold,
                onTap: () async {
            ...
            _settingRow(Icons.person_outline_rounded, 'Edit Profile',
                ...
                onTap: () => _showEditProfile()),
            _divider(),
            _settingRow(Icons.logout_rounded, 'Logout', 'Sign out of StudyAI',
                AppColors.error,
                onTap: () => _confirmLogout(), isDestructive: true),
          ])),
          const SizedBox(height: 16),
          Center(
              child: Text('StudyAI v1.0.0 · Made with ❤️ for students',
                  style: AppTextStyles.label.copyWith(fontSize: 11))),
        ]));
  }

  Widget _settingRow(IconData icon, String title, String subtitle, Color color,
                  ...
                  GlowButton(
                      ...
                      onPressed: () async {
                        ...
                        try {
                          ...
                          setModalState(() => error = 'Something went wrong');
                        }
                      }),
                ]))));
  }
...
class _LeaderboardScreenState extends State<_LeaderboardScreen> {
  ...
  // FIX: Use IntrinsicHeight + crossAxisAlignment stretch so all three podium
  // cards grow to the same height as the tallest one, preventing overflow.
  Widget _buildPodium() {
  ...
  // FIX: Replaced fixed `height` with `minHeight` via ConstrainedBox so the
  // card can grow if content exceeds the minimum, eliminating the overflow.
  Widget _podiumCard(Map<String, dynamic> user, int rank, double minHeight) {
    ...
    return ConstrainedBox(
      ...
      child: Container(
          ...
          decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(16),
              border: Border.all(
                  color: isMe ? AppColors.violet : color.withValues(alpha: 0.3),
                  width: isMe ? 2 : 1)),
...
// FIX: Wrapped Column in SingleChildScrollView so content can scroll when
// the keyboard pushes up, preventing the 28px bottom overflow.
// ══════════════════════════════════════════
...
class _BottomSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      // FIX: Use Padding (not Container) at the top level so the keyboard
      // inset is applied before layout, giving the scroll view room to work.
      padding:
      ...
      child: Container(
          ...
          // FIX: SingleChildScrollView lets the sheet content scroll if the
          // keyboard reduces available vertical space.
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              // Handle
              Container(
