...
// ══════════════════════════════════════════
// NOTES SCREEN
// Gate is applied ONLY before opening generate flow.
// List view, viewer, delete — all unchanged.
// ══════════════════════════════════════════
...
class _NotesScreenState extends State<NotesScreen>
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        ...
        SafeArea(child: Column(children: [
          ...
          Expanded(
            child: _loading
                ? const Center(child: NotesListSkeleton())
                : _error != null
                    ? _buildError()
                    : _notes.isEmpty
                        ? _buildEmpty()
  ...
  Widget _buildHeader() => Padding(
    ...
    child: Row(children: [
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text('${_notes.length} note${_notes.length == 1 ? '' : 's'} generated',
  ...
  Widget _buildNotesList() => FadeTransition(
    opacity: _fadeAnim,
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(
              ...
              Row(children: [
                ...
                if (chapters > 0) _chip('$chapters chapters', AppColors.cyan),
                const Spacer(),
                Text(date,
  ...
  // ══════════════════════════════════════════
  // GATE CHECK — runs before opening generate
  // 1. Device check (mobile blocked)
  // 2. Key / free-tier check
  // 3. Open generate screen if all clear
  // ══════════════════════════════════════════
  void _openGenerateFlow() async {
    // 1. Block mobile devices
    ...
    if (!hasKey && freeUsed && mounted) {
      // Free tier exhausted, no key — show paywall
    ...
    if (!hasKey && !freeUsed && mounted) {
      // First-time user — show onboarding with skip option
    ...
    // 3. All clear — open generate screen
    if (!mounted) return;
    final result = await Navigator.push<bool>(context,
      PageRouteBuilder<bool>(
  ...
  void _openNoteViewer(String id, String title) {
    Navigator.push(context,
      fadeSlideRoute(_NoteViewerScreen(noteId: id, title: title)));
  }
  ...
  String _modeLabel(String mode) {
    switch (mode) {
      case 'single': return 'Single Chapter';
      case 'multiple': return 'Multi Chapter';
      case 'full': return 'Full Document';
      default: return mode;
    }
  }
}
...
// ══════════════════════════════════════════
// GENERATE NOTES SCREEN — unchanged
// ══════════════════════════════════════════
    ...
    ShaderMask(
      ...
      child: const Text('Upload PDF',
        style: TextStyle(fontSize: 26, fontWeight: FontWeight.w800,
          color: Colors.white, fontFamily: 'Georgia'))),
    const SizedBox(height: 8),
    const Text('Upload your textbook or notes PDF\nto generate AI study notes',
      style: AppTextStyles.sub, textAlign: TextAlign.center),
    const SizedBox(height: 40),
    if (_error != null) ...[
      buildErrorBanner(_error!),
  ...
  Widget _buildGeneratingStep() => SizedBox(
    ...
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      GlassCard(child: Row(children: [
        ...
        Expanded(child: Text(
          'This may take 1-3 minutes depending on PDF size.',
...
// ══════════════════════════════════════════
// NOTE VIEWER — unchanged from original
// ══════════════════════════════════════════
            ...
            itemBuilder: (_, i) {
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    gradient:
                        sel ? AppColors.primaryGrad : null,
      ...
      Expanded(child: SingleChildScrollView(
        ...
        child: Column(
          ...
          _buildFormattedNotes(
            _chapters[_selectedChapter]['notes']
                    as String? ??
                ''),
          const SizedBox(height: 40),
        ]))),
