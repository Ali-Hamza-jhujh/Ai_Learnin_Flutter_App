          ...
          ShaderMask(
              ...
              child: const Text('AI Analytics',
                  style: TextStyle(
                      ...
                      color: Colors.white,
                      fontFamily: 'Georgia'))),
          const Text('Exam prediction & performance', style: AppTextStyles.sub),
        ]),
        const Spacer(),
        GestureDetector(
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            GlassCard(
              ...
              Container(
                  ...
                  child: const Text('cd backened/MLModels\npython app.py',
                      style: TextStyle(
...
class _DashboardTabState extends State<_DashboardTab> {
  ...
  Future<void> _predict() async {
    ...
    try {
      final res =
          await MLService.predictScore(targetSubject: _subjectCtrl.text.trim());
      if (mounted)
        setState(() {
      ...
      if (mounted)
        setState(() {
          _predError = e.message;
          _predicting = false;
        });
    } catch (_) {
      if (mounted)
        setState(() {
          _predError = 'Prediction failed';
          _predicting = false;
        });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final overall =
        (widget.dashboard['performance']?['overall']) as Map<String, dynamic>?;
    ...
    return SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (hasData && overall != null) ...[
            ...
            Row(children: [
              _miniStat('Tests', '$totalTests', const Color(0xFF48C6EF)),
              const SizedBox(width: 10),
              _miniStat(
                  'Avg Score',
                  '${(overall['averageScore'] as num?)?.toStringAsFixed(1) ?? '0'}%',
                  _scoreColor(
                      (overall['averageScore'] as num?)?.toDouble() ?? 0)),
              const SizedBox(width: 10),
              _miniStat('Grade', overall['overallGrade'] as String? ?? 'N/A',
                  AppColors.gold),
              const SizedBox(width: 10),
              _miniStat(
                  'Pass Rate',
  ...
  Widget _buildNoDataBanner() => Container(
      ...
      decoration: BoxDecoration(
          ...
          border: Border.all(color: const Color(0xFF48C6EF).withOpacity(0.3))),
      child: Row(children: [
        ...
        const Expanded(
          ...
          SizedBox(height: 4),
          Text(
              'Take quizzes to unlock AI predictions\nand performance analytics.',
              style: AppTextStyles.sub),
        ])),
      ]));
  ...
  Widget _buildTrendCard(Map<String, dynamic>? overall) {
    ...
    return GlassCard(
      ...
      const SizedBox(height: 14),
      Row(children: [
        _trendStat('🏆 Best', '${high.toStringAsFixed(0)}%', AppColors.success),
        const SizedBox(width: 16),
        _trendStat('📉 Lowest', '${low.toStringAsFixed(0)}%', AppColors.error),
        const SizedBox(width: 16),
        _trendStat(
            '✅ Pass Rate', '${passRate.toStringAsFixed(0)}%', AppColors.cyan),
      ]),
    ]));
  }

  Widget _trendStat(String label, String value, Color color) => Expanded(
          child: Column(children: [
...
class _PerformanceTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (performance == null) {
      return const Center(
          child: Text('No performance data yet.', style: AppTextStyles.sub));
    }
    ...
    final history = (performance!['scoreHistory'] as List<dynamic>? ?? [])
        ...
        .toList();
    final diffBreakdown =
        performance!['difficultyBreakdown'] as Map<String, dynamic>? ?? {};
          ...
          if (best != null || worst != null) ...[
            Row(children: [
              if (best != null)
                Expanded(
                    child:
                        _subjectHighlight('🏆 Best', best, AppColors.success)),
              if (best != null && worst != null) const SizedBox(width: 12),
              if (worst != null)
                Expanded(
                    child: _subjectHighlight(
  ...
  Widget _subjectHighlight(
      String label, Map<String, dynamic> subj, Color color) {
...
class _StudyPlanTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (recommendations.isEmpty) {
      return const Center(
          child: Text('No recommendations yet.', style: AppTextStyles.sub));
    }
